...
props['pegasus.file.cleanup.scope'] = 'deferred'
props.write()
...
rc2 = ReplicaCatalog()
rc2.add_replica(site="local", lfn="subsubwf1.yml", pfn=Path(__file__).parent.resolve() / "subsubwf1.yml")
rc2.add_replica(site="local", lfn="input.txt", pfn=Path(__file__).parent.resolve() / "input.txt")
...
# Sub-subworkflow1 will also include a job to output k3.txt. Assume this is
...
with open("output_ssw1.map", "w") as f:
    f.write("""k1.txt {}/newoutput/k1.txt pool="local"\n""".format(os.getcwd()))
...
rc2.add_replica(site="local", lfn="output_ssw1.map", pfn=Path(__file__).parent.resolve() / "output_ssw1.map")
output_map_ssw1 = File("output_ssw1.map")

input_file = File("input.txt")
k1_out = File("k1.txt")
k3_out = File("k3.txt")
sswf1 = Workflow("subsubworkflow-1")
k1 = Job(keg)\
...
sswf1.add_jobs(k1, ls1, k3)
sswf1.write("subsubwf1.yml")

# --- SubWorkflow 1 ----

# Subworkflow 1 will produce a k4.txt, but mostly connects ssw1 to the main WF

with open("output_sw1.map", "w") as f:
    f.write("""k1.txt {}/newoutput/k1.txt pool="local"\n""".format(os.getcwd()))
    f.write("""k4.txt {}/newoutput/k4.txt pool="local"\n""".format(os.getcwd()))
...
k4_out = File("k4.txt")
k4 = Job(keg)\
        .add_args("-i", input_file, "-o", k4_out, "-T", 5)\
        .add_inputs(input_file)\
        .add_outputs(k4_out)
...
wf1.add_replica_catalog(rc2)

j1 = SubWorkflow("subwf1.yml", _id="subwf1")
j1.add_args('-Dpegasus.dir.storage.mapper.replica.file=output_sw1.map')

j1a = SubWorkflow("subsubwf1.yml", _id="subsubwf1")
j1a.add_args('-Dpegasus.dir.storage.mapper.replica.file=output_ssw1.map')


j1a.add_planner_args(verbose=3, output_sites=['local'])\
        .add_inputs(input_file, output_map_ssw1)\
        .add_outputs(k1_out, stage_out=True)

j1.add_planner_args(verbose=3, output_sites=['local'])\
        .add_inputs(output_map_sw1)\
        .add_outputs(k1_out, stage_out=True)
